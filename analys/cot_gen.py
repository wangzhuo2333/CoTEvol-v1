...
output_file = os.path.join(output_path, f"t_s1K_cot.pkl")
...
for i in range(0, len(data), batch_size):
    ...
    for j, choice in enumerate(outputs.choices):
        batch_input[j]['output'] = choice.text.strip()
        dps.append(batch_input[j])

    # responses = [output.choices[0].text for output in outputs]
