...
def eval(llm, tokenizer, data_name, args):
    ...
    for epoch in range(max_func_call):
        ...
        if args.use_vllm:
            ...
            # n=2
            outputs = [[o.text.strip() for o in output.outputs] for output in outputs]
        else:
        ...
        # 初始化两个 end_prompts 列表，分别存每个解
        num_solutions = len(outputs[0])  # 假设每个样本都有一样数量的解
        end_prompts = [[] for _ in range(num_solutions)]

        # process all outputs
        remain_prompts = [[] for _ in range(num_solutions)]
        remain_codes = [[] for _ in range(num_solutions)]
        for (i, query), output_list in zip(current_prompts, outputs):
            for j, output in enumerate(output_list):
                output = output.rstrip()
                new_query = query + output

                if args.prompt_type == "cot":
                    end_prompts[j].append((i, new_query))  # 分别存入不同列表中
                elif args.prompt_type == "pal":
                    remain_prompts[j].append((i, new_query))
                    if "```python" in output:
                        output_code = extract_program(new_query)
                    else:
                        output_code = output
                    remain_codes[j].append(output_code)
                else:
                    end_prompts[j].append((i, new_query))
        
        for z in range(len(end_prompts)):
            ...
            end_prompts[z].extend(remain_prompts)
            # sort by idx
            end_prompts[z] = sorted(end_prompts[z], key=lambda x: x[0])

            # remove input_prompt from end_prompt
            codes = []
            assert len(input_prompts) == len(end_prompts[z])
            for i in range(len(input_prompts)):
                _, end_prompt = end_prompts[z][i]
                code = end_prompt[z].split(input_prompts[i])[-1].strip()
            ...
            all_samples, result_json, col_0 = evaluate(
                samples=all_samples,
                data_name=data_name,
            ...
            # return result_json
        result = [any(bits) for bits in zip(*col)]
        col_means= np.array(result).mean()
        mean_score = int(np.round(col_means * 100))
        print('BATCH ACC:',mean_score)
