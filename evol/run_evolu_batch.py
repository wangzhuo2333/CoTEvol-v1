...
# CUDA_VISIBLE_DEVICES=0,1 vllm serve /inspire/hdd/WORKSPACE_ID/public-project/USER_ID/Qwen2.5-7B-Instruct --tensor-parallel-size 2 --max-model-len 32768 --enforce-eager --port 8001
@dataclass
...
def main():
    ...
    elif args.data_name == "deepmath":
        rep_name = "final_answer"
    else:
    ...
    for i in range(0, len(idxs), args.batch_size):
        ...
        def get_batch_data(data, ids):
            problems, answers, use_ids = [], [], []
            for idx in ids:
                dp = data[idx]
                problem = dp["problem"]
                if "answer" in dp:
                ...
                use_ids.append(idx)
            return problems, answers, use_ids

        problems, answers, use_ids = get_batch_data(train_data, ids)
